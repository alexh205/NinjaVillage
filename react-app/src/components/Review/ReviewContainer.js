      ...
      <div className="flex flex-col md:mr-20 mr-12">
        <div className="mt-6">
          ...
          <div className="flex flex-row mt-1">
            ...
            {ratingAvg ? (
              <div className="flex flex-row text-base whitespace-nowrap">
                {ratingAvg.toFixed(1)}
                <p className="hidden md:flex text-base ml-1">out of 5.0</p>
              </div>
            ) : (
              !ratingAvg && (
                <div className="flex flex-row whitespace-nowrap text-base">
                  0 <p className="hidden md:flex text-base ml-1">out of 5.0</p>
                </div>
              )
            )}
          </div>
          {product && product.productReviews.length === 1 ? (
            ...
            <div className="text-sm text-gray-500 mt-1">0 rating</div>
          )}
        </div>
        {user &&
        product?.ownerId &&
        ...
        user.id === product?.ownerId ? null : !user ? (
          <div className="flex flex-col border-t border-b mt-3">
            ...
            <button
              className="mt-2 mb-4 self-center text-sm bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold py-1 px-1 sm:px-6 border border-gray-600 rounded shadow whitespace-nowrap"
          ...
          product?.productReviews && (
            <div
              ...
              <button
                ...
                onClick={() => history.push(`/reviews/${product.id}`)}>
        ...
        <div>
          ...
          <div>
            {product?.productReviews.map((review, i) => (
              <Review review={review} product={product} user={user} key={i} />
            ))}
          </div>
        </div>
      </div>
