  ...
  return (
    <div className='flex flex-row border-t border-double mx-3'>
        ...
        <div className='flex flex-col md:mr-20 mr-12'>
            <div className='mt-6'>
                <div className='md:text-2xl text-lg font-bold'>Customer reviews</div>
            ...
            {user && product && product.ownerId && product.productReviews && user.id === product.ownerId ? null : !user ? (<div className='flex flex-col border-t border-b mt-3'>
                ...
                <button className='mt-2 mb-4 self-center text-sm bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold py-1 px-6 border border-gray-600 rounded shadow ' onClick={()=> history.push('/login')}>{!user && 'Sign in to write a review'}</button>
            </div>): product && product.productReviews && (<div className='flex flex-col border-t border-b mt-3'>
                <div className='mt-4 font-semibold text-lg'>Review this product</div>
                <div className='mt-2 text-sm'>Share your thoughts with other customers</div>
                <button disabled={product.productReviews.find(review => review.owner.id === user.id)} className='mt-2 mb-4 self-center md:text-sm text-[10px] bg-white hover:bg-gray-100 text-gray-800 font-semibold py-1 md:px-6 px-2 border border-gray-400 rounded shadow' onClick={()=> history.push(`/reviews/${product.id}`)}>Write a customer review</button>
            </div>)}
        </div>
        <div className='flex flex-col mt-5'>
            <div>
                <div className='md:text-2xl text-lg font-bold mb-2'>User reviews</div>
                <div>
