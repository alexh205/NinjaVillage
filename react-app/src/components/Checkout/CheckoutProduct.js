...
const CheckoutProduct = ({product}) => {
  ...
  return (
    <div className="flex flex-row ml-4 my-2">
      <img
        src={productImageArr.length > 0 ? productImageArr[0].url : null}
        alt="product"
      ...
      <div className="flex flex-col mt-5">
        <div className="text-sm md:text-lg font-bold">{product.title}</div>
        <div className="text-sm font-bold text-orange-700">
          <p>${product.price}</p>
        </div>
        <div className="flex">
          <p className="text-sm text-gray-500">Sold by:</p>
          <p className="ml-1 text-sm text-gray-500">{product.brand}</p>
        </div>
