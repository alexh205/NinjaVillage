...
export const WishListContainer = () => {
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-3 border-[2px] p-2">
          ...
          <div className="col-span-2 mx-4">
            <div className="flex flex-col">
                ...
                <div
                  ...
                  onClick={() => {
                    if (activeList?.name !== 'Wish List') {
                      const data = dispatch(removeWishListThunk(activeList.id));
                      if (data) {
                        alert(`List '${activeList.name}' was deleted`);
                      }
                    } else {
                      alert("'Wish List' can't be deleted!");
                    }
                    setActiveList('');
                  }}>
                  Delete List
              ...
              <div>
                {activeList &&
                  activeList.listProducts.map((product, i) => (
                    <WishListProd
                      key={i}
                      product={product}
      ...
      <footer
        className={`${
          activeList && activeList.listProducts.length <= 3 ? 'hidden' : 'block'
        }`}>
        <a
          href="#header"
